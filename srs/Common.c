#define LOG_CLASS "Common"
#include "Samples.h"

#define GZNSCY_STUN_URL "stun:coturn.%s:3478"
#define GZNSCY_STUN_URL_POSTFIX "gznscy.com"

PSampleConfiguration gSampleConfiguration = NULL;
CHAR gSrsOffer[MAX_SESSION_DESCRIPTION_INIT_SDP_LEN + 1];
...
VOID onDataChannelMessage(UINT64 customData, PRtcDataChannel pDataChannel, BOOL isBinary, PBYTE pMessage, UINT32 pMessageLen)
{
    ...
    if (retStatus != STATUS_SUCCESS) {
        DLOGI("[CyIpc] dataChannelSend(): operation returned status code: 0x%08x \n", retStatus);
    }
}
...
STATUS signalingClientStateChanged(UINT64 customData, SIGNALING_CLIENT_STATE state)
{
    ...
    DLOGV("Srs client state changed to %d - '%s'", state, pStateStr);
...
STATUS signalingClientError(UINT64 customData, STATUS status, PCHAR msg, UINT32 msgLen)
{
    ...
    DLOGW("Srs client generated an error 0x%08x - '%.*s'", status, msgLen, msg);

    // We will force re-create the srs client on the following errors
...
//    CHK_STATUS(deserializeSessionDescriptionInit(pSignalingMessage->payload, pSignalingMessage->payloadLen, &answerSessionDescriptionInit));
    answerSessionDescriptionInit.type = SDP_TYPE_ANSWER;
    STRCPY(answerSessionDescriptionInit.sdp, pSignalingMessage->payload);
...
STATUS sendSignalingMessage(PSampleStreamingSession pSampleStreamingSession, PSignalingMessage pMessage)
{
    ...
    if (pMessage->messageType == SIGNALING_MESSAGE_TYPE_ANSWER) {
        ...
        DLOGP("[Srs offer to answer] %" PRIu64 " ms", pSampleConfiguration->signalingClientMetrics.signalingClientStats.offerToAnswerTime);
    }
                                    ...
                                    PSampleStreamingSession* ppSampleStreamingSession)
{
    ...
    // add by mlz
//    CHK_STATUS(cyConfigurationCreate(&pSampleConfiguration->pConfiguration));
//    CHK_STATUS(cyIpcCreate(&pSampleConfiguration->pIpc, pSampleConfiguration->pConfiguration));
//    CHK_STATUS(cyIpcStart(pSampleConfiguration->pIpc));
    ...
    STRCPY(videoTrack.streamId, "srsVideoStream");
    STRCPY(videoTrack.trackId, "srsVideoTrack");
    ...
    STRCPY(audioTrack.streamId, "srsVideoStream");
    STRCPY(audioTrack.trackId, "srsAudioTrack");
...
STATUS createSampleConfiguration(PCHAR channelName, SIGNALING_CHANNEL_ROLE_TYPE roleType, BOOL trickleIce, BOOL useTurn, UINT32 logLevel, PSampleConfiguration* ppSampleConfiguration)
{
    STATUS retStatus = STATUS_SUCCESS;
    ...
    if (NULL != getenv(ENABLE_FILE_LOGGING)) {
        ...
        if (retStatus != STATUS_SUCCESS) {
            DLOGW("[CyIpc] createFileLogger(): operation returned status code: 0x%08x", retStatus);
        } else {
...
STATUS initSignaling(PSampleConfiguration pSampleConfiguration, PCHAR clientId)
{
    ...
    // add by mlz
    CHK_STATUS(initLocalPeer(pSampleConfiguration, pSampleConfiguration->clientInfo.clientId));
    ...
    // Logging this here since the logs in srs library do not get routed to file
    DLOGP("[Srs Post offer] %" PRIu64 " ms", signalingClientMetrics.signalingClientStats.describeCallTime);
...
STATUS getIceCandidatePairStatsCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    CHK_WARN(pSampleConfiguration != NULL, STATUS_NULL_ARG, "[CyIpc] getPeriodicStats(): Passed argument is NULL");
...
STATUS pregenerateCertTimerCallback(UINT32 timerId, UINT64 currentTime, UINT64 customData)
{
    ...
    CHK_WARN(pSampleConfiguration != NULL, STATUS_NULL_ARG, "[CyIpc] pregenerateCertTimerCallback(): Passed argument is NULL");
...
STATUS sessionCleanupWait(PSampleConfiguration pSampleConfiguration)
{
    ...
    while (!ATOMIC_LOAD_BOOL(&pSampleConfiguration->interrupted)) {
        ...
        if (ATOMIC_LOAD_BOOL(&pSampleConfiguration->sampleStreamingSessionList[0]->terminateFlag)) {
        ...
        // Check if we need to re-create the srs client on-the-fly
        if (ATOMIC_LOAD_BOOL(&pSampleConfiguration->recreateSignalingClient)) {
            ...
            if (STATUS_SUCCEEDED(retStatus)) {
                ...
                printf("[KVS Common] recreating srs Client\n");
...
//        // Check the srs client state and connect if needed
...
STATUS signalingMessageReceived(UINT64 customData, PReceivedSignalingMessage pReceivedSignalingMessage)
{
    ...
    switch (pReceivedSignalingMessage->signalingMessage.messageType) {
        case SIGNALING_MESSAGE_TYPE_OFFER:
            ...
            MUTEX_LOCK(pSampleConfiguration->streamingSessionListReadLock);
            pSampleConfiguration->sampleStreamingSessionList[0] = pSampleStreamingSession;
            MUTEX_UNLOCK(pSampleConfiguration->streamingSessionListReadLock);
        ...
        case SIGNALING_MESSAGE_TYPE_ANSWER:
            ...
            // add by mlz
//            CHK_STATUS(cySetRemoteDescription(pSampleConfiguration->pIpc->pPeerConnection, pReceivedSignalingMessage->signalingMessage.payload));
        ...
        case SIGNALING_MESSAGE_TYPE_ICE_CANDIDATE:
             ...
             * submit the srs message into the corresponding streaming session.
             */
            if (!peerConnectionFound) {
        ...
        default:
            DLOGD("Unhandled srs message type %u", pReceivedSignalingMessage->signalingMessage.messageType);
            break;
    }
...
STATUS initLocalPeer(PSampleConfiguration pSampleConfiguration, PCHAR clientId)
{
    STATUS retStatus = STATUS_SUCCESS;
    ...
    CHK(pSampleConfiguration != NULL && clientId != NULL, STATUS_NULL_ARG);

    MEMSET(&offerSessionDescriptionInit, 0x00, SIZEOF(RtcSessionDescriptionInit));

    CHK_STATUS(createSampleStreamingSession(pSampleConfiguration, clientId, TRUE, &pSampleStreamingSession));

    // add by mlz
//    CHK_STATUS(cyCreateOffer(pSampleConfiguration->pIpc->pPeerConnection, &offer));
//    CHK_STATUS(cyRtcSetLocalDescription(pSampleConfiguration->pIpc->pPeerConnection->pRtcConnection, offer));
//    CHK_STATUS(signalingStorageLocalOffer(pSampleConfiguration->signalingClientHandle, offer));
    ...
    STRCPY(gSrsOffer, offerSessionDescriptionInit.sdp);
