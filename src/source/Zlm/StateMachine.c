...
StateMachineState SIGNALING_STATE_MACHINE_STATES[] = {
    {
        ...
        SIGNALING_STATE_NEW | SIGNALING_STATE_DESCRIBE | SIGNALING_STATE_GET_TOKEN,
        ...
        SIGNALING_STATE_GET_TOKEN | SIGNALING_STATE_NEW | SIGNALING_STATE_DESCRIBE,
...
//            STRNCPY(pSignalingClient->answerDescription.channelArn, pSignalingClient->pChannelInfo->pChannelArn, MAX_ARN_LEN);
//            pSignalingClient->answerDescription.channelArn[MAX_ARN_LEN] = '\0';
...
STATUS executeDescribeSignalingState(UINT64 customData, UINT64 time)
{
    ...
    PROFILE_CALL_WITH_T_OBJ(retStatus = pSignalingClient->pCredentialProvider->getCredentialsFn(pSignalingClient->pCredentialProvider,
        &pSignalingClient->pAwsCredentials),
        pSignalingClient->diagnostics.offerToAnswerTime, "Post offer call");
    ...
    PROFILE_CALL_WITH_T_OBJ(retStatus = describeChannel(pSignalingClient, time), pSignalingClient->diagnostics.offerToAnswerTime,
                            "Post offer to signaling call");
