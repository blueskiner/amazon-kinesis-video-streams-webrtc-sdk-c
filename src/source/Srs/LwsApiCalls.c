...
extern CHAR gSrsOffer[MAX_SESSION_DESCRIPTION_INIT_SDP_LEN + 1];

#define SERVICE_CALL_WHIP 201
...
//    CHK_STATUS(setRequestHeader(pRequestInfo, SRS_HEADER_HOST, 0, SRS_HEADER_HOST_VALUE, 0));
...
//    CHK_STATUS(setRequestHeader(pRequestInfo, AWS_SIG_V4_HEADER_AMZ_DATE, 0, dateTimeStr, 0));
    CHK_STATUS(setRequestHeader(pRequestInfo, "Content-Type", 0, "application/gSdp;charset=UTF-8", 0));
        ...
        case LWS_CALLBACK_RECEIVE_CLIENT_HTTP_READ:
            ...
            if (dataSize != 0) {
                ...
                if (pLwsCallInfo->callInfo.callResult != SERVICE_CALL_RESULT_OK &&
                    pLwsCallInfo->callInfo.callResult != SERVICE_CALL_WHIP) {
...
//    CHK(pHostEnd == NULL || *pHostEnd == '/' || *pHostEnd == '?', STATUS_INTERNAL_ERROR);
    ...
    *pHostEnd = '\0';
    ...
    while (retVal >= 0 && !gInterruptedFlagBySignalHandler && pLwsCallInfo->callInfo.pRequestInfo != NULL &&
...
//    UINT64 messageTtl;
    ...
    STRCAT(url, "/rtc/v1/whip/?app=live&stream=livestream");

    // Create the request info with the body
    CHK_STATUS(createRequestInfo(url, gSrsOffer, "", pSignalingClient->pChannelInfo->pCertPath, NULL, NULL,
    ...
    CHK((SERVICE_CALL_RESULT) ATOMIC_LOAD(&pSignalingClient->result) == SERVICE_CALL_WHIP &&
        resultLen != 0 &&
        pResponseStr != NULL,
...
//    CHK(pSignalingClient->answerDescription.channelStatus != SIGNALING_CHANNEL_STATUS_DELETING, STATUS_SIGNALING_CHANNEL_BEING_DELETED);
