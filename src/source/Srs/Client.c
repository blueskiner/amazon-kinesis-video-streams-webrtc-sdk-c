...
STATUS createRetryStrategyForCreatingSignalingClient(PSignalingClientInfo pClientInfo, PKvsRetryStrategy pKvsRetryStrategy)
{
    ...
    if (pClientInfo->signalingRetryStrategyCallbacks.createRetryStrategyFn == NULL ||
        ...
        DLOGV("Using exponential backoff retry strategy for creating srs client");
                           ...
                           PAwsCredentialProvider pCredentialProvider, PSIGNALING_CLIENT_HANDLE pSignalingHandle)
{
    ...
    DLOGI("Creating Srs Client Sync");
    ...
    while (TRUE) {
        ...
        // Wait before attempting to create srs client
...
CleanUp:
    if (STATUS_FAILED(retStatus)) {
        DLOGE("Create srs client API failed with return code [0x%08x]", retStatus);
        freeSignaling(&pSignalingClient);
    } else {
        PROFILE_WITH_START_TIME_OBJ(startTime, pSignalingClient->diagnostics.createClientTime, "Create srs client");
...
PUBLIC_API STATUS signalingStorageLocalOffer(SIGNALING_CLIENT_HANDLE signalingClientHandle, PCHAR offer)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PSignalingClient pSignalingClient = FROM_SIGNALING_CLIENT_HANDLE(signalingClientHandle);
    CHK(pSignalingClient != NULL && NULL != offer, STATUS_NULL_ARG);

    MEMSET(gSrsOffer, 0x00, ARRAY_SIZE(gSrsOffer));
    STRCPY(gSrsOffer, offer);
