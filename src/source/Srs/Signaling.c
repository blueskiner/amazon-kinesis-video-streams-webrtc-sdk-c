                     ...
                     PAwsCredentialProvider pCredentialProvider, PSignalingClient* ppSignalingClient)
{
    ...
    CHK_STATUS(threadpoolCreate(&pSignalingClient->pThreadpool, pClientInfo->signalingClientInfo.zlmMessagesMinimumThreads,
                                pClientInfo->signalingClientInfo.zlmMessagesMaximumThreads));
#endif
...
STATUS freeSignaling(PSignalingClient* ppZlmClient)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    PSignalingClient pSignalingClient;

    CHK(ppZlmClient != NULL, STATUS_NULL_ARG);

    pSignalingClient = *ppZlmClient;
...
STATUS signalingSendMessageSync(PSignalingClient pSignalingClient, PSignalingMessage pZlmMessage)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    BOOL removeFromList = FALSE;

    CHK(pSignalingClient != NULL && pZlmMessage != NULL, STATUS_NULL_ARG);
    CHK(pZlmMessage->peerClientId != NULL && pZlmMessage->payload != NULL, STATUS_INVALID_ARG);
    CHK(pZlmMessage->version <= SIGNALING_MESSAGE_CURRENT_VERSION, STATUS_SIGNALING_INVALID_SIGNALING_MESSAGE_VERSION);

    // Store the signaling message
    CHK_STATUS(signalingStoreOngoingMessage(pSignalingClient, pZlmMessage));
    removeFromList = TRUE;

    // Perform the call
    CHK_STATUS(sendLwsMessage(pSignalingClient, pZlmMessage->messageType, pZlmMessage->peerClientId, pZlmMessage->payload,
                                 pZlmMessage->payloadLen, pZlmMessage->correlationId, 0));
    if (pZlmMessage->messageType == SIGNALING_MESSAGE_TYPE_ANSWER) {
    ...
    if (pZlmMessage->messageType == SIGNALING_MESSAGE_TYPE_OFFER) {
...
CleanUp:
    ...
    if (removeFromList) {
        signalingRemoveOngoingMessage(pSignalingClient, pZlmMessage->correlationId);
    }
...
//    CHK_STATUS(acceptZlmStateMachineState(
//            pSignalingClient, ZLM_STATE_READY | ZLM_STATE_CONNECT | ZLM_STATE_CONNECTED | ZLM_STATE_DISCONNECTED));
...
//    ATOMIC_STORE(&pSignalingClient->result, (SIZE_T) SERVICE_CALL_RESULT_ZLM_RECONNECT_ICE);
...
//    if (pStateMachineState->state == ZLM_STATE_READY || pStateMachineState->state == ZLM_STATE_CONNECTED) {
//        CHK_STATUS(zlmStateMachineIterator(pSignalingClient, curTime + ZLM_REFRESH_ICE_CONFIG_STATE_TIMEOUT, pStateMachineState->state));
//    }
...
STATUS signalingStoreOngoingMessage(PSignalingClient pSignalingClient, PSignalingMessage pZlmMessage)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    BOOL locked = FALSE;
    ...
    CHK_STATUS(signalingGetOngoingMessage(pSignalingClient, pZlmMessage->correlationId, pZlmMessage->peerClientId, &pExistingMessage));
    CHK(pExistingMessage == NULL, STATUS_SIGNALING_DUPLICATE_MESSAGE_BEING_SENT);
    CHK_STATUS(stackQueueEnqueue(pSignalingClient->pMessageQueue, (UINT64) pZlmMessage));
...
STATUS signalingGetOngoingMessage(PSignalingClient pSignalingClient, PCHAR correlationId, PCHAR peerClientId, PSignalingMessage* ppZlmMessage)
{
    ENTERS();
    STATUS retStatus = STATUS_SUCCESS;
    ...
    CHK(pSignalingClient != NULL && correlationId != NULL && ppZlmMessage != NULL, STATUS_NULL_ARG);
