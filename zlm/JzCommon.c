...
int sample_system_init()
{
    ...
    DLOGD("sample_system_init start\n");

    ret = IMP_ISP_Open();
    if (ret < 0) {
        DLOGE("failed to open ISP\n");
        return -1;
    }

    ret = IMP_ISP_AddSensor(&sensor_info);
    if (ret < 0) {
        DLOGE("failed to AddSensor\n");
        return -1;
    }

    ret = IMP_ISP_EnableSensor();
    if (ret < 0) {
        DLOGE("failed to EnableSensor\n");
        return -1;
    }

    ret = IMP_System_Init();
    if (ret < 0) {
        DLOGE("IMP_System_Init failed\n");
        return -1;
    }
    ...
    if (ret < 0) {
        DLOGE("IMP_ISP_EnableTuning failed\n");
        return -1;
    }
    ...
    if (ret < 0) {
        DLOGE("failed to set running mode\n");
        return -1;
    }
#endif
#if 0
    ...
    if (ret < 0) {
        DLOGE("failed to set sensor fps\n");
        return -1;
    }
#endif

    DLOGD("ImpSystemInit success\n");
...
int sample_framesource_init()
{
    ...
    //for (i = 0; i < FS_CHN_NUM; i++) {
        if (chn[i].enable) {
            ...
            if (ret < 0) {
                DLOGE("IMP_FrameSource_CreateChn(chn%d) error !\n", chn[i].index);
                return -1;
            }

            ret = IMP_FrameSource_SetChnAttr(chn[i].index, &chn[i].fs_chn_attr);
            if (ret < 0) {
                DLOGE("IMP_FrameSource_SetChnAttr(chn%d) error !\n", chn[i].index);
                return -1;
            }
        }
    //}
    ...
    for (i=0; i<FS_CHN_NUM; i++) {
        if (chn[i].enable) {
            ...
            ret = IMP_Encoder_SetDefaultParam(&channel_attr, chn[i].payloadType, S_RC_METHOD,
                    ...
                    (S_RC_METHOD == IMP_ENC_RC_MODE_FIXQP) ? 35 : -1, uTargetBitRate);
            if (ret < 0) {
                DLOGE("IMP_Encoder_SetDefaultParam(%d) error !\n", chnNum);
                return -1;
            }
#ifdef LOW_BITSTREAM
            ...
            case IMP_ENC_RC_MODE_INVALID:
                DLOGE("unsupported rcmode:%d, we only support fixqp, cbr vbr and capped vbr\n", rcAttr->attrRcMode.rcMode);
                return -1;
            }
#endif
            ...
            if (ret < 0) {
                DLOGE("IMP_Encoder_CreateChn(%d) error !\n", chnNum);
                return -1;
            }

            ret = IMP_Encoder_RegisterChn(chn[i].index, chnNum);
            if (ret < 0) {
                DLOGE("IMP_Encoder_RegisterChn(%d, %d) error: %d\n", chn[i].index, chnNum, ret);
                return -1;
            }
        }
    }
...
int sample_encoder_exit(void)
{
    ...
    for (i = 0; i <  FS_CHN_NUM; i++) {
        if (chn[i].enable) {
            ...
            if (ret < 0) {
                DLOGE("IMP_Encoder_Query(%d) error: %d\n", chnNum, ret);
                return -1;
            }
            ...
            if (chn_stat.registered) {
                ...
                if (ret < 0) {
                    DLOGE("IMP_Encoder_UnRegisterChn(%d) error: %d\n", chnNum, ret);
                    return -1;
                }

                ret = IMP_Encoder_DestroyChn(chnNum);
                if (ret < 0) {
                    DLOGE("IMP_Encoder_DestroyChn(%d) error: %d\n", chnNum, ret);
                    return -1;
                }
            }
        }
...
int sample_framesource_exit()
{
    ...
    for (i = 0; i <  FS_CHN_NUM; i++) {
        if (chn[i].enable) {
            ...
            if (ret < 0) {
                DLOGE("IMP_FrameSource_DestroyChn(%d) error: %d\n", chn[i].index, ret);
                return -1;
            }
        }
    }
...
int sample_system_exit()
{
    ...
    DLOGD("sample_system_exit start\n");
    ...
    if(ret < 0){
        DLOGE("IMP_ISP_DisableTuning failed\n");
        return -1;
    }
    ...
    DLOGD("sample_system_exit success\n");
...
int sample_framesource_streamon()
{
    ...
    if (chn[0].enable) {
        ...
        if (ret < 0) {
            DLOGE("IMP_FrameSource_EnableChn(%d) error: %d\n", ret, chn[0].index);
            return -1;
        }
    }
...
int sample_framesource_streamoff()
{
    ...
    if (chn[0].enable){
        ...
        if (ret < 0) {
            DLOGE("IMP_FrameSource_DisableChn(%d) error: %d\n", ret, chn[0].index);
            return -1;
        }
    }
