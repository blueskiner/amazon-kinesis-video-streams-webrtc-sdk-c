...
CHAR gZlmOffer[MAX_SESSION_DESCRIPTION_INIT_SDP_LEN + 1];
...
STATUS initializePeerConnection(PSampleConfiguration pSampleConfiguration, PRtcPeerConnection* ppRtcPeerConnection)
{
    ...
    // Set the  STUN server
    PCHAR pGznscyStunUrlPostFix = GZNSCY_STUN_URL_POSTFIX;
    SNPRINTF(configuration.iceServers[0].urls, MAX_ICE_CONFIG_URI_LEN, GZNSCY_STUN_URL, pGznscyStunUrlPostFix);
...
//    CHK_STATUS(cyIpcStart(pSampleConfiguration->pIpc));
    ...
    STRCPY(videoTrack.streamId, "zlmVideoStream");
    STRCPY(videoTrack.trackId, "zlmVideoTrack");
    ...
    STRCPY(audioTrack.streamId, "zlmVideoStream");
    STRCPY(audioTrack.trackId, "zlmAudioTrack");
...
STATUS initSignaling(PSampleConfiguration pSampleConfiguration, PCHAR clientId)
{
    ...
    // add by mlz
    CHK_STATUS(initLocalPeer(pSampleConfiguration, clientId));
    ...
    // Logging this here since the logs in zlm library do not get routed to file
    DLOGP("[Zlm Post offer] %" PRIu64 " ms", signalingClientMetrics.signalingClientStats.describeCallTime);
...
STATUS signalingMessageReceived(UINT64 customData, PReceivedSignalingMessage pReceivedSignalingMessage)
{
    ...
    switch (pReceivedSignalingMessage->signalingMessage.messageType) {
        ...
        case SIGNALING_MESSAGE_TYPE_ANSWER:
            ...
            DLOGP("[Zlm offer to answer] %" PRIu64 " ms", pSampleConfiguration->signalingClientMetrics.signalingClientStats.offerToAnswerTime);
            break;
...
//    PCHAR offer;
    ...
    STRNCPY(gZlmOffer, offerSessionDescriptionInit.sdp, STRLEN(offerSessionDescriptionInit.sdp));
